...
WEMOD_DECK_URL := "https://github.com/wemod-deck/wemod-deck-ahk"
...
GetGameInfo(URL)
{
    ...
    RegExMatch(oHTTP.ResponseText, "meta_images/games/(\d+)/", GameId)
    RegExMatch(oHTTP.ResponseText, "<h1 class=""view-title"">(.*) cheats</h1>", GameName)
    RegExMatch(oHTTP.ResponseText, "cheats/([a-z0-9\-]+)-trainers", GameSlug)

    Return {Id: GameId1, Name: GameName1, Slug: GameSlug1}
}
...
{
    URL := "https://api.wemod.com/v3/games/" GameInfo.Id "/trainer?gameVersions=&locale=en-US&v=2"
...
GenerateTrainerScript(GameInfo, Trainer)
{
...
Gui, New, , [Wemod Deck] %GameName%
Gui, Font, s8
Gui, Add, StatusBar, , Wemod trainer version: %ReleasedAt%
Gui, Font, s12
...
Gui, Add, Button, gCloseSwicd x+10, Close SWICD
    ...
    FileDelete, % A_WorkingDir "\trainers\" GameInfo.Slug "-trainer.ahk"
    FileAppend, %FullScript%, % A_WorkingDir "\trainers\" GameInfo.Slug "-trainer.ahk"
...
ButtonGenerate:
{
    ...
    {
        AccessToken := ReadAccessToken()
        GameInfo := GetGameInfo(GameUrl)

        Trainer := GetTrainerData(GameInfo, AccessToken)
        GenerateTrainerScript(GameInfo, Trainer)
        MsgBox, % "Saved to: trainers\" GameInfo.Slug "-trainer.ahk"
    }
